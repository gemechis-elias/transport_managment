...
class _PininputState extends State<Pininput> {
  ...
  Future<void> loadTOHiveFromData() async {
    ...
    await box.put('bus_queue', json.encode(busQueueList));
  }

  Future<void> _loadPinCode() async {
    // Open the Hive box for 'pin_code'
    final pinBox = await Hive.openBox<String>('pin');

    // Get the pin code from the Hive box
    final pin = pinBox.get('pin');

    setState(() {
      pinCode = pin ?? ''; // Use empty string if pin is null
    });
